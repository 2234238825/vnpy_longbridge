...
# 兼容 NumPy 2.0：empyrical 仍引用已移除的 np.NINF
...
from vnpy.trader.object import OrderData, TradeData, BarData, TickData, HistoryRequest
...
class BacktestingEngine:
    ...
    def log_trade(self, trade: TradeData) -> None:
        """
        打印一笔成交日志，标明买入/卖出方向、数量与价格。
        """
        action_map: dict[tuple, str] = {
            (Direction.LONG, Offset.OPEN): "买入开仓",
            (Direction.SHORT, Offset.CLOSE): "卖出平仓",
            (Direction.SHORT, Offset.OPEN): "卖出开空",
            (Direction.LONG, Offset.CLOSE): "买入平空",
        }
        action: str = action_map.get(
            (trade.direction, trade.offset), str(trade.direction)
        )
        self.output(
            "【成交】{}  {}  {}股 @ {}".format(
                trade.datetime, action, trade.volume, trade.price
            )
        )
...
def load_bar_data(
    ...
    return get_datafeed().query_bar_history(req)
...
def load_tick_data(
    ...
    ticks: list[TickData] = database.load_tick_data(symbol, exchange, start, end)
    if ticks:
        return ticks
    ...
    return get_datafeed().query_tick_history(req)
